...
function readReportRows(worksheet) {
  ...
  worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    rows.push({
      ...
      vkName: cellValue(row, REPORT_COLS.vk_name),
...
function pairBonGroup(rows) {
  ...
  if (devices.length > 0 && wgItems.length === 0) {
    if (wgReview.length > 0) {
      // Geraet(e) vorhanden, aber der einzige WG-Artikel im Bon ist eine
      // "Wertgarantie GS"-Zeile ohne GKZ-Treffer - nicht automatisch paaren
      // (unklar), aber auch nicht stillschweigend verwerfen.
      devices.forEach((r) => review.push({ reason: 'wg-gs-unklar', rows: [r] }));
    }
    // Geraet ohne (normalen) WG-Artikel: laut Anleitung normaler Verkauf,
    // nicht Teil des Wertgarantie-Reportings -> ignorieren.
    return { pairs, review };
  }
  if (devices.length === 0 && wgItems.length === 0) {
    return { pairs, review };
