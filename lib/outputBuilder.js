...
function buildReviewWorkbookV2(review) {
  ...
  if (vermEntries.length) {
    const sheet = setupReviewSheet(
      ...
      'Diese Zeilen sind bereits in der Einspieldatei enthalten (siehe Bonnummer/Position). Bitte Vermittlernummer ermitteln (z.B. ueber die MA Liste/Verkaeufername) und dort direkt eintragen.',
  ...
  if (wgOhneGeraetEntries.length) {
    const sheet = setupReviewSheet(
      workbook,
      'WG ohne Geraet',
      'Fuer diese Wertgarantie-Artikel wurde kein passendes Geraet gefunden ("Gebraucht Nachtrag" laut Anleitung). Bitte Geraetedaten recherchieren (Salesforce/AX/Rueckfrage im Geschaeft), gelb markierte Felder ausfuellen und die komplette Zeile in die Einspieldatei einfuegen.',
  ...
  if (ambiguousEntries.length) {
    const sheet = setupReviewSheet(
      workbook,
      'Mehrdeutige Paarung',
      'Fuer diese Bons konnte die Zuordnung Geraet <-> Wertgarantie-Artikel nicht eindeutig automatisch bestimmt werden. Bitte anhand der "Kandidat"-Zeilen entscheiden, die "AUSFUELLEN"-Zeile ergaenzen und in die Einspieldatei kopieren.',
  ...
  if (gsEntries.length) {
    ...
    sheet.addRow(['Diese Wertgarantie-Variante ("Wertgarantie GS") ist in der PG-Matching-Tabelle nicht hinterlegt, daher konnte kein Geraetekennzeichen ermittelt werden. Bitte manuell pruefen und passende Zeile in die Einspieldatei eintragen.']);
    ...
    for (const r of flat) {
      sheet.addRow([r.bonnummer, r.pgr === 'Wertgarantie GS' ? 'WG-GS-Artikel' : 'Geraet', r.artikelbezeichnung, r.pgr, r.anzahl, r.umsatz, r.seriennr, r.nachname]);
    }
  }
