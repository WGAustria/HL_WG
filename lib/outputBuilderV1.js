...
async function buildOutput(reportWorkbook, { anthropic, model, dateFrom, dateTo } = {}) {
  ...
  for (const pair of pairs) {
    ...
    // Fehlende Vermittlernummer: die Zeile ist bereits (mit Luecke) in der
    ...
    // Netzbetreiber-Geraet: anders als bei fehlender Vermittlernummer wird
...
function buildReviewWorkbookV2(review) {
  ...
  if (netzbetreiberEntries.length) {
    const sheet = setupReviewSheet(
      workbook,
      'Netzbetreiber-Gerät',
      'Mobilfunk-Vorwahl in der Modellbezeichnung erkannt - bitte manuell prüfen, bevor die Zeile in die Einspieldatei übernommen wird.',
      ['Bonnummer']
    );
    for (const entry of netzbetreiberEntries) {
      ...
      highlightCells(row, [DATEN_COL.Modellbezeichnung]);
    }
  }
